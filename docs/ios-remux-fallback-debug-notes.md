...
- 当前结论写于 2026-05-02。
- 这份文档现在只保留为历史排障记录；当前 iOS 运行时已经删除 FFmpeg remux 回退路径，并保留修复后的实时 HLS 代理路径。
- Rust `packaging.offline_build` 仍可接收 remux diagnostics 作为离线对照输入，但那只是比较和回归素材，不代表 remux 运行时被恢复。
...
3. 流式 FFmpeg remux 回退路径也不能把 qn125 救回来；它同样会在 `init.mp4 + seg-0.m4s` 之后报 `-12927`。
4. 同一份 qn125 样本经过离线 Apple 风格 HLS 重打包后，可以在最小 AVPlayer harness 中起播；历史上整文件先下载完再播也验证过可以工作。

这四点合在一起，足以说明：

- qn125 不是“天生不能被 AVPlayer 播放”。
- 当前失败集中在 app 内实时分片交付路径，而不是一个简单的 init box 命名问题。
...
- 不能把向后兼容的 Dolby Vision 8.x 流直接写成 `CODECS="dvh1..."`。Apple 推荐的写法是：
	- `CODECS` 写基础层，例如 `hvc1...`
	- `SUPPLEMENTAL-CODECS` 写 Dolby Vision 增强信息，例如 `dvh1.08.09/db4h`
- 不能让 `playurl` 的粗粒度 hint 覆盖 init sample entry 的真实解析结果。`hvcC` / `dvvC` / color metadata 才是更高优先级的生成依据。
- audio/video media playlist 的 `TARGETDURATION` 不能各写各的；Apple validator 会把这种差异判成必须修复问题。

这类问题的本质已经不是“AVPlayer 会不会解 Dolby Vision”，而是“我们有没有把向后兼容的 Dolby Vision 正确生成成 Apple 期望的 HLS 形式”。
...
- 也就是说，问题已经从“patch 一个 init box”收敛为“当前实时分段打包不满足 AVPlayer 对 Apple 风格 HLS/fMP4 的要求”。
...
- 如果上游拿不到，就需要单独的正式打包层来重组视频 segment，而不是继续沿用当前实时 `sidx -> HLS BYTERANGE` 代理伪装。
